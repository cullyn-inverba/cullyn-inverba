...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \]%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{itemize}
      \item This is a consequence of the Pythagorean theorem, since the \hyperref[tbd]{\dlink{basis vectors}}  \\ \(\bm{e}_1\), \(\bm{e}_2\), and \(\bm{e}_3\) are \hyperref[Orthogonality]{\dlink{orthogonal}} \hyperref[Unit Vectors]{\dlink{unit vectors}}.
    \end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \jjj{Cross (vector, directed area) product}: denoted by the symbol \jjj{\(\bm{\times} \)}, indicating a binary operation on two vectors in \emph{three-dimensional space \(\mathbb{R}^3 \)}.
      \begin{itemize}
        \item Given two \hyperref[tbd]{\dlink{linearly independent}} vectors \tbm{a} and \tbm{b}, then the cross product \(\bm{a}\times\bm{b}\) produces a new vector that is \emph{orthogonal} to both \tbm{a} and \tbm{b}, or \emph{normal} to the plane containing them.
...
\section{Linear Subspace}\label{Linear Subspace}
\begin{itemize}
