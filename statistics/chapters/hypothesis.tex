...
\begin{itemize}
  ...
  \begin{itemize}
    \item \ddd{Parametric statistics}: based on the assumptions wherein sample data originate from a population that can be adequately modeled by a probability distribution with \bBb{a fixed set of parameters}.
    \item \ddd{Nonparametric statistics}: based on \OOO{relaxed assumptions} surrounding of parametric tests, e.g., underlying distribution less important, presence of outliers, or lower specificity of parameters.
    ...
    \begin{table}[h]
      ...
      \begin{tabular}{rl}
        ...
        \end{tabular}
    \end{table}
    \item Important applications of nonparametric statistics with no direct correlate involve \hyperref[Subsection: Primer: Permutation Testing]{\dlink{permutation testing}} and \hyperref[Section: Cross-Validation]{\dlink{cross-validation}}.
    ...
    \begin{multicols}{2}
      ...
      \end{itemize}
    \end{multicols}
    \item In general, use \OOO{parametric} methods when \OOO{possible} and \bBb{nonparametric} methods when \bBb{necessary}.
  \end{itemize}
