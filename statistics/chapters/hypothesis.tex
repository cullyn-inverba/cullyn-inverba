...
\begin{itemize}
  ...
  \begin{itemize}
    \item \ddd{False positive (type I error) \(p = \alpha\)}: an \textbf{\fff{incorrect rejection}} of a \ttt{true \(H_\nil\)}.
      \begin{itemize}
        \item \ddd{True positive \(p = 1-\beta\)}: a \rrr{\textbf{correct rejection}} of a \fff{false \(H_\nil\)}.
      \end{itemize}
    \item \ddd{False negative (type II error) \(p= \beta\)}: an \textbf{\XXX{incorrect non-rejection}} of a \fff{false \(H_\nil\)}.
      \begin{itemize}
        \item \ddd{True negative \(p = 1-\alpha\)}: a \bbb{\textbf{correct non-rejection}} of a \ttt{true \(H_\nil\)}.
      \end{itemize}
    \item \ddd{``Overlap''}: the overlap between the \(H_\nil\) and the \(H_a\) is where error of either type is found.
      \begin{itemize}
    ...
    \vspace{24pt}  
    \begin{center}
      \hspace{-26pt}\Image{\columnwidth}{chapters/images/errors.png}
    \end{center}
  \end{itemize}
