...
\begin{itemize}
  \item Reviewing dependent and independent variables (parameters):
    \begin{itemize}
      \item \ddd{\yyy{Dependent variable \(y\)}}: the variable you are trying to explain; the \yyy{output} of a function.
      \item \ddd{\xxx{Independent variables \(x_n\)}}: the variables that potentially explain the dependent variable; the \xxx{input(s)} to a function.
  ...
  \item \ddd{Models}: a simplified system made of the composition of concepts which are used to help know, understand, or simulate a subject the model represents.
    \begin{itemize}
      \item \ddd{\fff{Residual (error) \(\varepsilon\)}}: the degree that features not explained by variables that make up the composition of models. 
  ...
  \begin{itemize}
    ...
    \item Common misinterpretations of \(p\)-values:
      \begin{itemize}
        \item \false{Incorrect}: 
          \begin{itemize}
            \item ``\textit{My \(p\)-value is \fff{0.02}, so the \fff{effect is present for 2\%} of the population.}''
            \item ``\textit{My \(p\)-value is \fff{0.02}, so there is a \fff{98\% chance} that my sample statistic equals the population parameter.}''
            \item ``\textit{My p-value is \fff{smaller than the threshold}, therefore the \fff{effect is real}.}''
          \end{itemize}
        \item \true{Correct}: 
        \begin{itemize}
          \item ``My \(p\)-value is \ttt{0.02}, therefore there is a \ttt{2\% chance} that there is \ttt{no effect} and my \ttt{sample statistic} was \ttt{due to sampling variability}, noise, small sample size, and/or systematic bias.''
        \end{itemize}
      \end{itemize}
    ...
    \item Given a Gaussian distribution, z-proportion (above/below) values are:
      \begin{itemize}
        \item \ddd{68.3\%} of the data are within \(\ddD{\sigma_1}\leftrightarrow \ddd{z = \pm 1}= \ddd{0.683}\) 
        \item \Ssec{95.5\%} of the data are within \(\ssec{\sigma_2}\leftrightarrow \Ssec{z = \pm 2} = \Ssec{0.955}\)
        \item \Tsec{99.7\%} of the data are within \(\tsec{\sigma_3}\leftrightarrow \Tsec{z = \pm 3} = \Tsec{0.997}\)
      \end{itemize}
  ...
  \begin{itemize}
    \item \ddd{Degrees of freedom (\(df,~\nu\))}: the number of values (\emph{parameters}) in the final calculation of a statistic that are \emph{free to vary}.
      \begin{itemize}
    ...
    \item Higher degrees of freedom generally indicate more \hyperref[Subsection: Statistical Power]{\dlink{power to reject}} the \(H_\nil\).
  ...
  \subsection{Primer: Cross-Validation}
    ...
    \item Cross-validation is used on models with one or more unknown parameters, wherein a dataset is used to fit the data to the parameter via optimization.
      \begin{itemize}
        ...
        \item A more in-depth discussion of over and underfitting is covered when discussing \hyperref[Subsection: Nested Models]{\dlink{nested models}}.
      \end{itemize}
    \item Cross-validation is often used when dealing with \hyperref[Chapter: Regression]{\dlink{regression}} and \hyperref[Section: Confidence Intervals]{\dlink{confidence intervals}}.
      \begin{itemize}
...
\section{T-Tests}
\begin{itemize}
  \item \ddd{Student's \(t\)-test}: a test statistic that follows a \hyperref[Subsection: Primer: Probability Distributions]{\ulink{student's t-distribution}}, i.e., a test for relatively small sample sizes with unknown variance. 
    \begin{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \]%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{itemize}
      \item Note: \(\sigma\) is assumed to be an appropriate \hyperref[Chapter: Analysis of Variance]{\dlink{measure of variability}}.
    \end{itemize}
